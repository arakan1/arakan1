# Hi, I’m Rakan 👋

- Master’s student in Computer Science at Virginia Tech  
- Full-Stack Software Engineer 
- Interested in building scalable backend systems, APIs, and data-driven applications

---

## 🧠 What I Do
- Design and build **backend services and RESTful APIs**
- Work extensively with **databases, transactions, and data integrity**
- Focus on **performance, scalability, and reliability**
- Build full-stack applications

---

## 🛠️ Tech Stack

Languages: Python, JavaScript, Java, SQL<br>
Backend: Node.js, Express, Flask, Django, RESTful APIs, JWT Authentication, Middleware, Input Validation<br>
Databases: MySQL, PostgreSQL, MongoDB | Query Optimization, Indexing, Normalization<br>
Frontend: React, Next.js, HTML5, CSS3, Responsive Design<br>
DevOps & Tools: Docker, Git, GitHub Actions (CI/CD), AWS, Linux, Postman, Vercel<br>
Concepts: Data Structures & Algorithms, System Design, OOP, Testing (pytest, Jest), API Security


---

## 📌 Selected Projects

- **Car Dealership Management System**  
full-stack platform with 20+ REST APIs, role-based access control, optimized SQL queries, and transactional workflows.

- **Email Sensitivity Detector**  
Flask-based  service using TinyBERT to detect sensitive content with high accuracy and low-latency inference.

- **Auto Sales Platform**  
Full-stack application
https://americanautosalesvirginia.com/

---

## 🌱 Currently Working On
- Building a secure, local-only marketplace platform for students to trade items, sublease apartments, and share events, restricted to verified .edu email addresses  
- System design and scalable architectures  
- Improving API reliability and security  

---

## 🌐 Let’s Connect
- Portfolio: https://rakanawwad.dev  
- LinkedIn: https://linkedin.com/in/rakanawwad  
- Email: rakanawwad48@gmail.com
